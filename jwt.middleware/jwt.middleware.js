...
const isAuth = (req, res, next) => {
  try {
    ...
    // Verify the token. Returns payload if the token is valid,
    //  otherwise throws an error
    const payload = jwt.verify(token, process.env.TOKEN_SECRET);
    // Add payload to the request object as req.payload for
    // use in next middleware or route
    req.payload = payload;
  ...
  // Export the middleware so that we can use it to create
  // protected routes
};
//EXPORT
